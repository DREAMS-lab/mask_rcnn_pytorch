...
if __name__ == '__main__':
    ...
    dataset = Dataset("./datasets/Crater/aug/", "./datasets/Crater/aug/", transforms=get_transform(train=True), readsave=True, include_name=False)
    dataset_test = Dataset("./datasets/Crater/image/", "./datasets/Crater/npy/", transforms=get_transform(train=False), readsave=False, savePickle=False, include_name=False)
    ...
    #dataset_test = Dataset("./datasets/Crater", transforms=get_transform(train=False), include_name=False, input_channel=input_c)
    # image_mean, image_std, _, _ = dataset.imageStat()
    image_mean = [0.26982665183805044, 0.26982665183805044, 0.26982665183805044]
    image_std = [0.1271129429255276, 0.1271129429255276, 0.1271129429255276]
    ...
    for epoch in range(init_epoch, init_epoch + num_epochs):
        save_param = "trained_param_crater/epoch_{:04d}.param".format(epoch)
        torch.save(mask_rcnn.state_dict(), save_param)
        # train for one epoch, printing every 10 iterations
        train_one_epoch(mask_rcnn, optimizer, data_loader, device, epoch, print_freq=50)
...
"""
from model import *
from crater_train import *
device = torch.device('cuda:1')
dataset = Dataset("./datasets/Crater/split/", "./datasets/Crater/split/", transforms=get_transform(train=True), include_name=False)
data_loader = torch.utils.data.DataLoader( dataset, batch_size=5, shuffle=True, num_workers=8, collate_fn=utils.collate_fn)
image_mean = [0.26982665183805044, 0.26982665183805044, 0.26982665183805044]
image_std = [0.1271129429255276, 0.1271129429255276, 0.1271129429255276]
mask_rcnn = get_model_instance_segmentation(2, image_mean, image_std, stats=True)
mask_rcnn.load_state_dict(torch.load("trained_param_crater/epoch_0099.param"))
mask_rcnn.to(device)
"""
