...
if __name__ == '__main__':
    ...
    dataset = Dataset("./datasets/hypolith/aug/", transforms=get_transform(train=True), include_name=False, input_channel=input_c)
    dataset_test = Dataset("./datasets/hypolith/aug/", transforms=get_transform(train=False), include_name=False, input_channel=input_c)
    ...
    save_param = "trained_param_hypolith/epoch_{:04d}.param".format(0)
    torch.save(mask_rcnn.state_dict(), save_param)
    for epoch in range(init_epoch, init_epoch + num_epochs):
        save_param = "trained_param_hypolith/epoch_{:04d}.param".format(epoch)
        print(save_param)
